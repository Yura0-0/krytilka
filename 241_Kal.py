...
def ptk_ppls(pix):  # постер 4 кал 12 конверт 1 2 5 календарь вымпел и биг магнит
    ...
    for ved in vedomosti:
        ...
        if int(id_region) > 100:
            ...
            papka_2 = os.path.join(path_krytilka, ved, "СКом", "01_Кадр_2_К")
            papka_3 = os.path.join(path_krytilka, ved, "СКом", "01_Кадр_3_К")
            papka_4 = os.path.join(path_krytilka, ved, "СКом", "01_Кадр_4_К")
            papka_5 = os.path.join(path_krytilka, ved, "СКом", "01_Кадр_5_К")
            jpg1 = sorted(os.listdir(papka_1))
            jpg2 = os.listdir(papka_2)
            jpg3 = os.listdir(papka_3)
            jpg4 = os.listdir(papka_4)
            jpg5 = os.listdir(papka_5)
            ...
            try:
                if (q1 != q2).any() or (q1 != q3).any() or (q1 != q4).any() or (q1 != q5).any():
                    init(autoreset=True)
                    print(Fore.RED + f'[**ERROR**] имена jpg в №_{ved} не совпадают:\n ')
                    continue
            except ValueError:
            ...
            for jpg in q1:
                ...
                if not name_id is mas_brak_fon:
                    ...
                    path_jpg_2 = os.path.join(papka_2, jpg)
                    path_jpg_3 = os.path.join(papka_3, jpg)
                    path_jpg_4 = os.path.join(papka_4, jpg)
                    path_jpg_5 = os.path.join(papka_5, jpg)

                    im1 = Image.open(path_jpg_1)
                    im2 = Image.open(path_jpg_2)
                    im3 = Image.open(path_jpg_3)
                    im4 = Image.open(path_jpg_4)
                    im5 = Image.open(path_jpg_5)
                    ...
                    im_vimpel = Image.open(r"241\ПТК_ППЛС\Калининград под старину\вымпел.jpg")
                    ...
                    # im_10x15_
                    im_10x15_.paste(im1, (-120, 0))  # Калиниград
                    path_out = os.path.join(path_create_papka, ved + "_P", f'{pp["в печать 1"]}', f'{name_id}.jpg')
                    im_10x15_.save(path_out, dpi=(300, 300), quality=95)
                    # вставка 3 фотки
                    im_v_shablon.paste(im_10x15_, (0, 1818))
                    ...
                    im_10x15_.paste(im2, (0, 0))
                    path_out = os.path.join(path_create_papka, ved + "_P", f'{pp["в печать 2"]}', f'{name_id}.jpg')
                    im_10x15_.save(path_out, dpi=(300, 300), quality=95)

                    path_out = os.path.join(path_create_papka, ved + "_P", f'{pp["в печать 5"]}', f'{name_id}.jpg')
                    im4.save(path_out, dpi=(300, 300), quality=95)

                    # вставка 3 фотки
                    im_v_shablon.paste(im4, (12, 25))
                    ...
                    # Магнит большой
                    basewidth = 1773
                    wpercent = (basewidth / float(im5.size[0]))
                    hsize = int((float(im5.size[1]) * float(wpercent)))
                    im5 = im5.resize((basewidth, hsize), Image.Resampling.LANCZOS)

                    im_magnit.paste(im5, (0, -70))

                    # нумерация
                    draw = ImageDraw.Draw(im_magnit)
                    font = ImageFont.truetype(r"C:\Windows\Fonts\Arial.ttf", 27)
                    draw.text((1570, 30), f"{name_id}", (255, 255, 255), font=font)
                    # save
                    path_out = os.path.join(path_create_papka, ved, f'{pp["Магнит"]}', f'{name_id}.jpg')
                    ...
                    im_vimpel.paste(tim, (1670, 2270), tim)
...
if __name__ == '__main__':
    ...
    Flg = 0
    print("241___ПТК под старину:  версия - 19.06.24")
    ...
    if d1.year == 2025 and d1.month == 7:
        print("[ERROR -~ 1476] Обратитесь в тех. поддержку")
        sys.exit(0)
